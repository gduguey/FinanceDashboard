"""Archive raw bank statements to Cloudflare R2 (S3-compatible), falling back to local disk.

Per `docs/accounting/architecture.md`'s caching rule, a raw statement is
the one thing under `data/` that isn't safely re-fetchable, so it gets
archived verbatim before anything else touches it. `StatementArchive` is
where that archive actually lives: Cloudflare R2 when `R2_ACCOUNT_ID` etc.
are set in `.env`, the same local `raw_statements/` directory as before
otherwise — so a fresh clone with no R2 bucket configured keeps working
exactly as it always has.

A deliberate duplicate of `trades.utils.statement_archive`, not a shared
import from it — see this module's own `io_utils.py`'s docstring for why
...
class StatementArchive:
    ...
    `importers.ingest._archive_raw_statement`, which forwards unvalidated
    form fields into it).
    """
    ...
    def list_relative_paths(self, pattern: str = "*") -> list[str]:
        ...
        ----------
        pattern
            An `fnmatch` pattern (e.g. `"*.csv"`, `"*/*/*.csv"`) matched
