...
function AccountKindTable() {
  const kinds: [string, string][] = [
    ...
    ['external_investment', "A placeholder tracking a brokerage account's value — see Investments below for the pull-from-Investments-or-set-manually choice it's created with."],
...
function OverviewTab() {
  return (
    <div className="max-w-3xl space-y-8 text-sm leading-relaxed text-foreground/90">
      <p>
        This app keeps track of your money in two separate ways, side by side. Think of them as two different
        notebooks that mostly don't talk to each other.
      </p>
      ...
      <div className="rounded-lg border border-foreground/10 bg-muted/40 px-4 py-3">
        ...
        <p className="mt-1 flex items-start gap-2 text-sm leading-relaxed">
          ...
          <span>
            ...
            added together — everything you own (bank accounts, investments, even something like a car you've typed
            in by hand) minus anything you owe, as one single number. Outside of that one number, the two sides run
            completely independently: nothing you do on the Money side changes your investments, and nothing you do
            with your investments changes your everyday budget.
          </span>
        </p>
      </div>
      ...
      <p>
        ...
        budgets, and goals work; the <span className="font-medium text-foreground">Investments</span> tab does the
        same for the brokerage side. Neither one is required reading for the other.
      </p>
    </div>
  )
}
    ...
    <div className="space-y-10">
      <div className="rounded-lg border border-foreground/10 bg-gradient-to-br from-muted/60 to-transparent p-5">
        ...
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
          ...
          Tags, Budget, Goals, Net Worth — is a different view onto one shared record of what actually happened in
          your real accounts. Nothing is duplicated or separately maintained: change how a transaction is categorized
          in one place, and every chart, budget, and goal that touches it updates the same way. This page walks
          through that shared foundation once, end to end, so the vocabulary and mechanics used everywhere else make
          sense the first time you see them.
        </p>
      </div>
      ...
      <Section id="importing" icon={Upload} title="Importing data">
        <p>
          ...
          page. Drag and drop a CSV export from one of your bank or card accounts, and a module in the backend reads
          it, translates whatever column names and codes that specific institution uses into this app's own shared
        ...
        <p>
          ...
          let alone across different banks. So each account type gets its own small, dedicated piece of code that
          knows exactly how to read its particular shape of file and turn it into the app's standard format. When
          you add an account whose CSV shape the app hasn't seen before, the Import page flags it with a warning
          instead of guessing — at that point, a new import module needs to be added in the backend code to teach
          the app that specific format before files from that account can be dropped in directly.
        </p>
        <p>
          ...
          <span className="font-medium text-foreground">Accounts</span> page instead. Every account has a{' '}
          <em>kind</em>, which tells the rest of the app what it represents:
        </p>
        <AccountKindTable />
        <p>
          ...
          choose whether it pulls its value from Investments or is tracked manually like any other account (useful
          for a friend-managed fund, or a brokerage this app doesn't sync with directly) — see the Investments tab
          for what "pulling from Investments" actually means. Either way, this is the one link between the two
          halves of the app, and it only flows in one direction: Net Worth reads a live number from Investments;
          nothing in Investments ever reads accounting data back.
        </p>
      </Section>
      ...
      <Section id="ledger" icon={GitBranch} title="The ledger & postings">
        ...
        />
        <p>
          Both rows share one <code className="rounded bg-muted px-1 py-0.5 text-xs">transaction_id</code> (that's
          how an importer builds the pair), both carry the same date and description, and the two amounts cancel
      ...
      <Section id="rules" icon={ArrowRightLeft} title="Transfer rules">
        ...
        <p>
          ...
          directions together as one action when it detects a likely transfer — see the next section. Rules
          themselves are authored and edited on the <span className="font-medium text-foreground">Rules</span> page.
        </p>
      </Section>
      ...
      <Section id="transfer-suggestions" icon={ArrowRightLeft} title="Auto-detected transfer suggestions">
        <p>
          ...
          as suggestions on the Rules page's Suggestions tab, each with a proposed rule for both directions, fully
          editable before you add either one — the heuristic can be wrong (two unrelated transactions that happen to
          share an amount), so nothing is ever applied automatically.
        </p>
        <p>
          A suggestion that's genuinely not a transfer doesn't have to keep coming back: dismissing it with{' '}
          <span className="font-medium text-foreground">Not relevant</span> archives it instead of discarding it —
          it stops being proposed, but stays listed (and restorable) in a small archive at the bottom of the page, in
      ...
      <Section id="duplicates" icon={CopyCheck} title="Duplicate detection">
        <p>
          ...
          duplicate is the opposite problem: the <em>same</em> real-world purchase imported twice into the same
          account, because it reached this app through two different sources (a CSV export and a statement PDF, say)
          that each gave it their own transaction id. The Import page's Duplicates tab scans for groups of two or
          more transactions on one account with a matching amount, close dates, and similar-enough descriptions, and
          sorts them least-certain-first so the ones needing the closest look come first.
        </p>
        <p>
          Nothing is merged automatically. Reviewing a group lets you pick which transaction to keep — the others
          are dropped entirely, both their legs — with an optional description override for the one you keep. The
          same <span className="font-medium text-foreground">Not relevant</span> dismiss-and-archive mechanism from
          transfer suggestions applies here too: a group that isn't actually a duplicate can be archived instead of
          reviewed every time, and restored later if you change your mind.
        </p>
      </Section>
      ...
      <Section id="categorizing" icon={Tags} title="Categorizing & tags">
        ...
        <Definition term="Category">
          ...
          postings it's used on. Categories, patterns, and a reference taxonomy all live on the{' '}
        ...
        <Definition term="Tag">
          ...
          tag spanning Gifts, Food & Drink, and Travel. Tags are managed on their own{' '}
      ...
      <Section id="automated-categorization" icon={Sparkles} title="Automated categorization">
        ...
        <ul className="ml-4 list-disc space-y-1">
          <li>
            ...
            subcategory. Needs an API key entered on the{' '}
            <span className="font-medium text-foreground">Settings</span> page first; the button is disabled
            without one.
          </li>
          <li>
      ...
      <Section id="goals" icon={Target} title="Goals">
        ...
        <p>Two optional automations build on top of this, and a goal's own contribution history is kept on a dedicated ledger tab so it's never just a running total you have to trust:</p>
        <ul className="ml-4 list-disc space-y-1">
          <li>
    ...
    <div className="space-y-10">
      <div className="rounded-lg border border-foreground/10 bg-gradient-to-br from-muted/60 to-transparent p-5">
        <p className="flex items-center gap-2 text-base font-semibold text-foreground">
          ...
          A separate module, built the same way
        </p>
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
      ...
      <Section id="inv-overview" icon={Compass} title="Overview">
        <p>
          ...
          Importing data section): when you create one, you choose whether it pulls its current value live from
          here, or is tracked manually instead. Pulling only works once Investments actually has synced data to
          pull — the account-creation form greys the option out and explains why if it doesn't yet. Past that one
          link, the two sides never read from each other.
        </p>
        <p>
      ...
      <Section id="inv-ledger" icon={History} title="Ledger & replay">
        <Definition term="Ledger">
          the complete history of every deposit, withdrawal, buy, sell, dividend, fee, and stock split ever
          recorded for this brokerage account, in one common format, in order. It's the investments side's
          counterpart to the Money side's own ledger — the one place data is durably written as a fact.
        </Definition>
        <Definition term="Replay">
          recomputing a result by walking through the ledger's history in order, instead of reading a value that
          was stored ahead of time. What you currently hold, what it cost, and what it's worth today are all worked
          out fresh this way every time a page loads — never a cached answer that could quietly go stale.
        </Definition>
        <p>
          Syncing pulls new events from your broker (via IBKR's Flex Web Service, the only source this app
          currently supports) and appends them to this history — it only ever adds, never edits a past event in
          place. The raw statement each sync fetches is archived exactly as received, so the derived ledger can
          always be thrown away and rebuilt from those originals if it's ever suspected of drifting out of sync.
        </p>
      </Section>

      <Section id="inv-lots" icon={Layers} title="Lots">
        <Definition term="Lot">
          one single purchase of a security, tracked on its own rather than blended into one running position. Buy
          the same stock on three different days and you have three lots, each with its own purchase date, price,
          and — once sold — its own realized gain. When shares are sold, the app consumes the{' '}
          <em>oldest</em> matching lots first (called FIFO — first in, first out), the same convention most brokers
          use for tax reporting.
        </Definition>
        <Definition term="Open lot">
          a purchase that still has shares remaining. Its gain or loss is <em>unrealized</em> — it isn't locked in
          yet, and moves with the current market price.
        </Definition>
        <Definition term="Closed lot">
          the part of a purchase that's been sold. It has its own sale price, holding period, and{' '}
          <em>realized</em> gain — final the moment the sale happens, and never affected by where the market goes
          afterward. A single sale can close multiple lots, or only part of one.
        </Definition>
      ...
      <Section id="inv-performance" icon={TrendingUp} title="Performance & benchmarks">
        <p>
          The Performance page exists to answer one question honestly: is investing actually working, separate
          from the fact that you keep putting money in? A rising balance is easy to mistake for a good decision,
          when most of the rise might just be a recent deposit landing. Every metric here is built to keep those
          two things apart.
        </p>
        <Definition term="XIRR">
          the single annual return rate that would exactly explain every deposit, withdrawal, and today's value,
          given exactly when each one happened. It measures the return on your actual money, timing included.
        </Definition>
        <Definition term="TWR (Time-Weighted Return)">
          the portfolio's investment return with deposit/withdrawal timing removed entirely — "how did the
          investments themselves perform," regardless of when money was added. This is what makes it fair to
          compare against a benchmark fund, which never had to guess when you'd add money.
        </Definition>
        <Definition term="NAV (Net Asset Value)">
          the value of one portfolio "unit," the same idea a mutual fund uses. A deposit buys more units, a
          withdrawal sells units, so the unit price itself only moves because of investment performance — never
          because money moved in or out. TWR is built by chaining this unit price's own growth over time.
        </Definition>
        <Definition term="Counterfactual">
          ...
          destination changes. The gap between what actually happened and a counterfactual is the real-dollar
          answer to "was this worth it."
        </Definition>
        <p>
          <span className="font-medium text-foreground">Dollar alpha vs. HYSA</span> is today's portfolio value
          minus today's HYSA counterfactual value — a positive number means investing produced more real dollars
          than parking the same deposits in savings would have.{' '}
          <span className="font-medium text-foreground">Growth of $100</span> makes the same comparison a different
          way: every line starts at exactly 100 and ignores contribution timing entirely, so it compares{' '}
          <em>rates</em> of growth rather than dollar outcomes.{' '}
      ...
      <Section id="inv-allocation" icon={PieChart} title="Allocation & cash">
        <Definition term="Drift">
          the gap between a holding's current share of the portfolio and the target share you've set for it.
          Positive means you're holding more of it than intended; negative means less.
        </Definition>
        <p>
          The Allocation page also tracks uninvested cash sitting in the account over time, and flags it once it's
          been sitting a while — a light flag past a week, a stronger one past two weeks — since idle cash is the
          one part of a portfolio that definitely isn't growing. Alongside the flag, it estimates what that same
          cash would be worth today had it been invested (in the benchmark, or at the HYSA rate) the moment it
          arrived instead, so the cost of leaving it uninvested is a real dollar figure, not just a vague sense that
          it's "sitting there."
        </p>
      ...
      <Section id="inv-taxes" icon={Percent} title="Taxes">
        ...
        <ul className="ml-4 list-disc space-y-1">
          <li>
            <span className="font-medium text-foreground">Report</span> — the year-by-year realized gains and
            dividends, an estimated tax bill, any flagged wash sales, and a live "if you sold everything today"
            preview.
          </li>
          <li>
            <span className="font-medium text-foreground">How it's taxed</span> — a plain-language explanation of
            what your chosen residency status actually means for dividends and capital gains.
          </li>
        </ul>
        <Definition term="Tax regime">
          which U.S. tax treatment applies — NRA (nonresident alien, e.g. F-1 student status) generally owes no
          U.S. tax on bank interest or on security sales at all; RESIDENT (e.g. H-1B, once the substantial-presence
          test is met) is taxed the same way a U.S. citizen is, on both. Chosen above the tab selector on the Taxes
          page, since it's the one setting that shapes everything in the report below it.
        </Definition>
        <Definition term="LTCG (Long-Term Capital Gains)">
          the lower tax rate a RESIDENT pays on a lot held longer than 365 days (or a qualified dividend), instead
          of their regular marginal rate. Never applies to an NRA, whose gains aren't U.S.-taxed at all regardless
          of how long anything was held.
        </Definition>
        <Definition term="Wash-sale">
          ...
          within 30 days before or after the sale. It's a mechanical proximity check meant to surface the risk, not
          an automatic adjustment to any number shown elsewhere on this dashboard.
        </Definition>
      </Section>
    </div>
  )
