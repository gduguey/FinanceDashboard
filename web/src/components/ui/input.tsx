...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  return (
    <InputPrimitive
      ...
      data-slot="input"
      className={cn(
        // `text-foreground` is explicit, not inherited — a caller wrapping
        // this in e.g. `<label className="text-muted-foreground">` (every
        // filter/form field in this app does) would otherwise leak that
        // grey into whatever the user actually typed, since plain form
        // controls inherit `color` from their parent by default.
