...
function SelectValue({
  ...
  // base-ui's `Select.Value` only auto-resolves a selected value's label when
  // `Select.Root` was given a data-driven `items` list — this codebase builds
  // options declaratively via `<SelectItem>` children instead, so without this,
  // every select would show the raw value (e.g. an id) once something is
  // selected rather than its display label. Pass the same `value -> label`
  // mapping used to render the `<SelectItem>`s here to fix that.
  ...
  return (
    ...
    >
      {items
        ? (value: unknown) =>
            value !== null && value !== undefined && items[String(value)] !== undefined
              ? items[String(value)]
              : placeholder
        : undefined}
    </SelectPrimitive.Value>
...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        // `text-foreground` is explicit, not inherited (see the same note
    ...
    <SelectPrimitive.Portal>
      ...
      >
        <SelectPrimitive.Popup
          ...
          className={cn(
            'relative isolate z-50 max-h-(--available-height) w-max min-w-(--anchor-width) max-w-[min(90vw,28rem)] origin-(--transform-origin) overflow-x-hidden overflow-y-auto rounded-lg bg-popover text-popover-foreground shadow-md ring-1 ring-foreground/10 duration-100 data-[align-trigger=true]:animate-none data-[side=bottom]:slide-in-from-top-2 data-[side=inline-end]:slide-in-from-left-2 data-[side=inline-start]:slide-in-from-right-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95',
            className,
          )}
          {...props}
        >
...
function SelectItem({ className, children, ...props }: SelectPrimitive.Item.Props) {
  return (
    <SelectPrimitive.Item
      ...
      {...props}
    >
      <SelectPrimitive.ItemText className="flex min-w-0 flex-1 truncate gap-2">{children}</SelectPrimitive.ItemText>
