...
// (usually lumpier) contribution history.
function buildCurve(contributions: GoalContribution[], goal: Goal): CurvePoint[] {
...
export function GoalDetailChart({ goal, contributions }: { goal: Goal; contributions: GoalContribution[] }) {
  const points = buildCurve(contributions, goal)
  const targetDateStr = goal.target_date.slice(0, 10)
        ...
        {points.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={260}>
            <LineChart data={points} margin={{ left: 8, right: 16 }}>
              ...
              />
              <ReferenceLine y={goal.target_amount} stroke={goal.color} strokeDasharray="4 4" label="Target" />
              <ReferenceLine x={targetDateStr} stroke="#94a3b8" strokeDasharray="2 2" label="Target date" />
