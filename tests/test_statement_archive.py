...
from trades.utils import statement_archive
from trades.utils.statement_archive import R2Credentials, StatementArchive
...
def test_local_fallback_round_trips_bytes(tmp_path) -> None:
    ...
    archive.write("ibkr/20260101T000000.xml", b"<FlexQueryResponse />")
    assert archive.read("ibkr/20260101T000000.xml") == b"<FlexQueryResponse />"
    assert (tmp_path / "ibkr" / "20260101T000000.xml").read_bytes() == b"<FlexQueryResponse />"


def test_local_fallback_exists_and_list_relative_paths(tmp_path) -> None:
    archive = StatementArchive(tmp_path, "statements/user-1", credentials=R2Credentials(_env_file=None))
    archive.write("20260101T000000.xml", b"a")
    archive.write("20260201T000000.xml", b"b")
    assert archive.exists("20260101T000000.xml") is True
    assert archive.exists("does-not-exist.xml") is False
    assert archive.list_relative_paths("*.xml") == ["20260101T000000.xml", "20260201T000000.xml"]
...
def test_local_fallback_read_all_yields_every_matching_file(tmp_path) -> None:
    ...
    archive.write("20260101T000000.xml", b"a")
    archive.write("20260201T000000.xml", b"b")
    assert sorted(archive.read_all("*.xml")) == [("20260101T000000.xml", b"a"), ("20260201T000000.xml", b"b")]
...
def test_r2_write_read_exists_and_list_use_the_prefixed_key(monkeypatch, tmp_path) -> None:
    ...
    archive = StatementArchive(tmp_path, "statements/user-1/ibkr", credentials=_configured_credentials())

    archive.write("20260101T000000.xml", b"<FlexQueryResponse />")

    assert fake_client.objects == {"statements/user-1/ibkr/20260101T000000.xml": b"<FlexQueryResponse />"}
    assert archive.read("20260101T000000.xml") == b"<FlexQueryResponse />"
    assert archive.exists("20260101T000000.xml") is True
    assert archive.exists("missing.xml") is False
    assert archive.list_relative_paths("*.xml") == ["20260101T000000.xml"]
