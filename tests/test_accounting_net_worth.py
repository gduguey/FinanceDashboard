...
def _posting(posting_id: str, transaction_id: str, account_id: str, amount: float, posted_at: str = "2026-06-01") -> dict:
    return {
...
CHECKING = Account(account_id="chase:checking:9579", name="Chase Checking", kind="checking", institution="Chase", currency="USD")
CREDIT_CARD = Account(account_id="chase:credit_card:8235", name="Chase Credit Card", kind="credit_card", institution="Chase", currency="USD")
SAVINGS = Account(account_id="sofi:savings:3680", name="SoFi Savings", kind="savings", institution="SoFi", currency="USD")
EUR_ACCOUNT = Account(account_id="bnp:checking:0001", name="BNP Checking", kind="checking", institution="BNP", currency="EUR")
VAULT = Account(
    account_id="sofi:savings:3680:vault:travel", name="Travel Vault", kind="vault", institution="SoFi",
    currency="USD", parent_account_id="sofi:savings:3680",
)
UNCATEGORIZED_EXPENSE = Account(account_id="uncategorized:expense", name="Uncategorized Expense", kind="expense_payee", institution="internal", currency="USD")
EXTERNAL_INVESTMENT = Account(account_id="external:interactive-brokers", name="Interactive Brokers", kind="external_investment", institution="external", currency="USD")
...
def test_net_worth_includes_a_vault_as_its_own_asset_row_without_double_counting() -> None:
    ...
    accounts = {SAVINGS.account_id: SAVINGS, VAULT.account_id: VAULT, UNCATEGORIZED_EXPENSE.account_id: UNCATEGORIZED_EXPENSE}
    summary = net_worth_summary(postings, accounts, [], date(2026, 6, 30))
    assert summary.assets == pytest.approx(1000.0)
...
def test_net_worth_adds_manually_entered_other_assets() -> None:
    summary = net_worth_summary(_postings(), {}, [OtherAsset(asset_id="car", name="Car", value=15000.0)], date(2026, 6, 30))
...
def test_net_worth_converts_a_eur_account_into_usd_display() -> None:
    postings = _postings(_posting("p1", "t1", "bnp:checking:0001", 1000.0), _posting("p2", "t1", "uncategorized:expense", -1000.0))
    accounts = {EUR_ACCOUNT.account_id: EUR_ACCOUNT, UNCATEGORIZED_EXPENSE.account_id: UNCATEGORIZED_EXPENSE}
    summary = net_worth_summary(postings, accounts, [], date(2026, 6, 30), display_currency="USD", eur_usd_rate=1.10)
...
def test_net_worth_converts_a_eur_other_asset_into_eur_display() -> None:
    summary = net_worth_summary(
        _postings(), {}, [OtherAsset(asset_id="flat", name="Paris flat", value=1000.0, currency="EUR")],
        date(2026, 6, 30), display_currency="EUR", eur_usd_rate=1.10,
    )
...
def test_net_worth_converts_a_usd_other_asset_into_eur_display() -> None:
    summary = net_worth_summary(
        _postings(), {}, [OtherAsset(asset_id="car", name="Car", value=1100.0, currency="USD")],
        date(2026, 6, 30), display_currency="EUR", eur_usd_rate=1.10,
    )
    assert summary.other_assets_total == pytest.approx(1000.0)


def test_net_worth_series_computes_one_point_per_date() -> None:
    postings = _postings(
        _posting("p1", "t1", "chase:checking:9579", 500.0, posted_at="2026-06-01"),
        _posting("p2", "t1", "uncategorized:expense", -500.0, posted_at="2026-06-01"),
        _posting("p3", "t2", "chase:checking:9579", 500.0, posted_at="2026-06-15"),
        _posting("p4", "t2", "uncategorized:expense", -500.0, posted_at="2026-06-15"),
    )
    accounts = {CHECKING.account_id: CHECKING, UNCATEGORIZED_EXPENSE.account_id: UNCATEGORIZED_EXPENSE}
    series = net_worth_series(postings, accounts, [], [date(2026, 6, 5), date(2026, 6, 20)])
    assert series == [(date(2026, 6, 5), pytest.approx(500.0)), (date(2026, 6, 20), pytest.approx(1000.0))]
